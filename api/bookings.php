...
// ── GET Available Slots ──
if ($method === 'GET' && isset($_GET['slots'])) {
    $date     = trim($_GET['date'] ?? '');
    $barberId = intval($_GET['barber_id'] ?? 0) ?: null;
    if (!$date) jsonResponse(false, 'Date is required.', [], 422);

    if ($barberId !== null) {
        $stmt = $db->prepare(
            "SELECT booking_time FROM bookings
             WHERE barber_id = ? AND booking_date = ? AND status NOT IN ('cancelled')"
        );
    ...
    jsonResponse(true, 'OK', ['booked_times' => $booked]);
}
...
switch ($method) {
    ...
    case 'POST':
        ...
        // Get service price
        $s = $db->prepare('SELECT id, price FROM services WHERE id = ? AND is_active = TRUE');
        $s->execute([$serviceId]);
        $service = $s->fetch();
        ...
        if ($auth) {
            ...
            if ($existingUser) {
                ...
                $insertUser = $db->prepare('INSERT INTO users (name, email, phone, password) VALUES (?, ?, ?, ?) RETURNING id');
        ...
        if ($barberId !== null) {
            ...
            // No specific barber: check if ALL barbers are booked at that slot
            $conflict = $db->prepare(
                'SELECT id FROM bookings
                WHERE booking_date = ? AND booking_time = ?
                  AND status NOT IN (\'cancelled\')'
            );
            $conflict->execute([$date, $time]);
        }
        if ($conflict->fetch()) {
