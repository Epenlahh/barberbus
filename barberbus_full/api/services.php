...
function handleServices(PDO $db, string $method, int $id): void {
    switch ($method) {
        ...
        case 'POST':
            ...
            $stmt = $db->prepare('INSERT INTO services (name, description, price, duration, category) VALUES (?,?,?,?,?) RETURNING id');
            $stmt->execute([$name, $b['description'] ?? '', $price, $duration, $b['category'] ?? 'haircut']);
            jsonResponse(true, 'Service created.', ['id' => $stmt->fetchColumn()]);
            break;
...
function handleBarbers(PDO $db, string $method, int $id): void {
    switch ($method) {
        ...
        case 'POST':
            ...
            $stmt = $db->prepare('INSERT INTO barbers (name, specialty, bio, experience, rating) VALUES (?,?,?,?,?) RETURNING id');
