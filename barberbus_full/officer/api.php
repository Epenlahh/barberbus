...
function requireOfficer(): array {
    ...
    if (!in_array($payload['role'] ?? '', ['admin','officer'])) {
...
switch ($action) {
    ...
    case 'queue':
        ...
        $stmt  = $db->prepare("
            SELECT b.id, b.booking_time, b.status, b.notes,
                   ...
                   s.name  AS service_name,   s.duration,
                   br.name AS barber_name,    br.id   AS barber_id,
                   FLOOR(EXTRACT(EPOCH FROM ((b.booking_date + b.booking_time::time) - NOW())) / 60)::int AS mins_until
            FROM bookings b
            JOIN users    u  ON b.user_id    = u.id
            JOIN services s  ON b.service_id = s.id
            LEFT JOIN barbers br ON b.barber_id = br.id
    ...
    case 'walkin':
        ...
        if ($existing) {
            ...
            $ins  = $db->prepare("INSERT INTO users (name,email,phone,password,role) VALUES (?,?,?,?,'user') RETURNING id");
        ...
        jsonResponse(true, 'Walk-in added to queue!', ['booking_id' => $ins2->fetchColumn()]);
        break;
    ...
    case 'stats':
        ...
        $next = $db->prepare("
            ...
            WHERE b.booking_date=? AND b.status IN ('pending','confirmed')" . $nextFilter . "
              AND b.booking_time >= CURRENT_TIME
    ...
    case 'notifications':
        ...
        $stmt  = $db->prepare("
            ...
            JOIN users u ON b.user_id=u.id
            JOIN services s ON b.service_id=s.id
            WHERE b.created_at >= NOW() - make_interval(secs => ?)
    ...
    case 'login':
        ...
        $stmt = $db->prepare("SELECT id,name,email,password,role FROM users WHERE email=? AND role IN ('admin','officer')");
        $stmt->execute([$email]);
        $user = $stmt->fetch();
        if (!$user || !password_verify($pass, $user['password'])) {
            jsonResponse(false, 'Invalid credentials or insufficient access.', [], 401);
        }
