...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        $serviceStmt = $pdo->prepare('SELECT id, name, price, duration FROM services WHERE id = ? AND is_active = TRUE');
    ...
    if ($barberId && !$error) {
        $barberStmt = $pdo->prepare('SELECT id, name FROM barbers WHERE id = ? AND is_active = TRUE');
    ...
    if (!$error) {
        $insert = $pdo->prepare('INSERT INTO bookings (user_id, barber_id, service_id, booking_date, booking_time, total_price, pay_method, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id');
        $insert->execute([
